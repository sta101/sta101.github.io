...
3. Let *A* be the event an individual orders from Dairy Queen. Let *B* be the event that the individual ordered and consumed 1 item that contained more than 6 grams of sugar. Is event *B* independent of *A*?
...
- Next compute the true mean using all the subway menu items in the `fastfood` data frame. Does your confidence interval capture the true mean? Use `set.seed(4)` when creating your bootstrap distribution.
...
5. Could you use Central Limit theorem to construct a confidence interval in exercise 4? Why or why not? Create a figure to support your claim.

```{r eval = F, echo = F}
subwaySample %>%
  ggplot(aes(x = cholesterol)) +
  geom_histogram() +
  labs(x = "Cholesterol", y = "Count", title = "Cholesterol in 12 subway menu items") +
  theme_bw()
